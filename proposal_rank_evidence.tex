...
\end{equation}
where $f_{kj}$ is the free-space propagator from slit $k$ to pixel $j$.  Because
$I_j$ is a linear function of $\rho$, the intensity matrix $\mathbf{D}$ (rows =
settings, columns = pixels) satisfies
\begin{equation}
    \mathrm{rank}(\mathbf{D}) \leq n^2 - 1
    \label{eq:rank_qm}
\end{equation}
for \emph{any} fixed slit subset, since $\rho$ has $n^2 - 1$ real degrees of
freedom (the $-1$ from $\mathrm{Tr}\,\rho = 1$).

Crucially, the $I_3 = 0$ condition of QM implies that three- and four-slit
intensity patterns are \emph{linear combinations} of two-slit patterns.  Therefore
the column space of $\mathbf{D}_3$ and $\mathbf{D}_4$ is contained within the
column space of $\mathbf{D}_2$, giving the stronger prediction
\begin{equation}
    \mathrm{rank}(\mathbf{D}_{\mathrm{all}}) = \mathrm{rank}(\mathbf{D}_2) = n^2 - 1.
    \label{eq:rank_equal}
\end{equation}
A theory with $I_3 \neq 0$ would produce 3- or 4-slit patterns with components
\emph{orthogonal} to the 2-slit subspace, violating Eq.~\eqref{eq:rank_equal}:
